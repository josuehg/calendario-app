...
with tab_fijo:
    ...
    fx_state = st.session_state.get("_fx_dialog")
    if fx_state:
        ed = fx_state.get("data")
        ...
        def _fx_dialog():
            ...
            cat = c1.selectbox("Categoría", cats,
                               ...
                               key="fx_cat")
            b_idx = 0
            if ed and ed.get("branch") in branch_opts:
                b_idx = branch_opts.index(ed["branch"])
            ...
            if ed:
                st.divider()
                st.caption(
                    "Al **desactivar** deja de generarse a futuro (los meses ya generados "
                    "se mantienen). **Eliminar** borra la plantilla; los gastos ya "
                    "generados quedan como sueltos."
                )
...
with tab_prox:
    ...
    mng = st.session_state.get("_gx_manage")
    if mng:
